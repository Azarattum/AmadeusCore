...
export default class Utils {
  ...
  public static log(text: string, type: LogType = LogType.INFO): void {
    ...
    switch (type) {
      case LogType.INFO: {
        console.log(prefix + "%c\x1b[1mi\x1b[0m " + text, "font-weight:bold;");
        break;
      }
      ...
      case LogType.WARNING: {
        console.warn(
          prefix + "\x1b[33m\x1b[1m%c!\x1b[0m \x1b[33m" + text + "\x1b[0m",
      ...
      case LogType.DIVIDER: {
        ...
        console.log(divider + text + divider + (text.length % 2 ? "=" : ""));
        break;
      }
  ...
  public static convertTo(proto: any, data: any): any {
    if (typeof proto === "string") {
      ...
      return typeof data === "bigint" ? data : BigInt(Number.parseInt(data));
      ...
      if (Array.isArray(proto)) {
        ...
        if (typeof data === "object" && data !== null) {
          ...
          for (let i = 0; i < data.length; i++) {
            let type = proto[i];
            if (typeof type === "undefined" && unitype) type = proto[0];
