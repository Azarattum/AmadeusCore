import { TrackPreview, Tracks } from "../track.interface";
...
export default abstract class Provider<T = any> extends Fetcher {
  public async *get(query: string, from: TrackSource = "search"): Tracks {
    const sources: Record<TrackSource, (query: string) => AsyncGenerator<T>> = {
      search: this.search,
      artist: this.artist,
    ...
    try {
      ...
      const err = e.toString() === "[object Object]" ? JSON.stringify(e) : e;
  ...
  protected abstract convert(track: T): TrackPreview | null;
